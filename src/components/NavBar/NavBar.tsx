...
const NavBar: React.FC<Props> = ({ navBar, setSection, setNavBar, deshabilitarMouseDown }) => {
  const disabled = () => {
  ...
  return (
    <div className="absolute z-50 h-full">
      ...
      <div className={`overflow-hidden ${openNav ? " w-48  p-4" : " w-0"}  h-full absolute z-50 transition-all bg-white border-r-2`}>
        <button className="border-2 p-2 rounded-lg text-orangeMedit border-orangeMedit text-base w-full my-4 bg-white font-bold hover:bg-orangeMedit hover:text-white transition-all" onClick={() => { setSection(1), disabled() }}>Actualizar Precios</button>
        <button className="border-2 p-2 rounded-lg text-orangeMedit border-orangeMedit text-base w-full my-4 bg-white font-bold hover:bg-orangeMedit hover:text-white transition-all" onClick={() => { setSection(2), disabled() }}>Subir Ofertas</button>
